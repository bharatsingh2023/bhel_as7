...
function createInFlowOutFlow(leftID, rightID) {
	...
	console.log("next Div ChildrenL  :: " + nextDivChildrenL.className);
	for (var childL of nextDivChildrenL.children) {
		if (childL.tagName != 'p' && document.getElementsByTagName(childL.tagName)[1].id == "amt") {

			var spanId = document.getElementsByTagName(childL.tagName)[1].id;
			console.log("child tagNameL id :: " + spanId);
			console.log("child tagNameL value :: " + document.getElementById(spanId).innerHTML);
	...
	for (var childR of nextDivChildrenR.children) {
		if (childR.tagName != 'p' && document.getElementsByTagName(childR.tagName)[1].id == "amtR") {

			var spanIdR = document.getElementsByTagName(childR.tagName)[1].id;
			console.log("child tagNameR id :: " + spanIdR);
			console.log("child tagNameR value :: " + document.getElementById(spanIdR).innerHTML);
			rightAmmount = document.getElementById(spanIdR).innerHTML;
		}
	}
	console.log("rightAmmount :: " + rightAmmount);
	//console.log("child tagName :: " + elementL.id);
	return leftAmmount + "," + rightAmmount;

	//console.log(" <===> children elementL :: " + children.id);
}
function drawArrow(elementL, elementR, isCalledFromReDrawPath, pathKey) {

	console.log("svgCount :: " + svgCount)
	//createInFlowOutFlow(elementL, elementR)
...
function createMapForSave(leftID, rightID, pathName) {
	...
	leftMap.set("leftRate", splitReturnVal[0]);
	...
	leftMap.set("rightRate", splitReturnVal[1]);
