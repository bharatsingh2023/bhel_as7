		...
		if (sides == "left") {
			...
			for (let x in secondArray) {
				...
				bbuhtml +=
					...
					"_L' class='labell'><div class='sublabell'><P id = 'desc" + secondArray[x].value + "L'> <B><I>" + secondArray[x].name + "</B></I>:" + secondArray[x].description + "</P>" +
					"<label id = 'qty" + secondArray[x].value + "Lb'>Qty.: </label> <span id = 'qty" + secondArray[x].value + "L'>" + secondArray[x].quantity
					+ "</span><br><label id = 'amt" + secondArray[x].value + "Lb'> Total ammount: Rs.</label> <span id = 'amt" + secondArray[x].value + "L'> " + secondArray[x].ammount
					+ "</span><br><label id = 'rate" + secondArray[x].value + "Lb'> Rate:</label> <span id = 'rate" + secondArray[x].value + "L'>" + secondArray[x].rate + "</span></div></div>";
				...
				if (catgarray != null && catgarray.length > 0) {
					...
					for (let catg in catgarray) {
						...
						subtxt_1 += "<Button id = 'handle_" + childIdL + "_L' ondblclick='toggleVisibility(&#39hiddenContainerDiv_" + childIdL + x + "&#39)' class = 'subhandleL' value='" + catgarray[catg].value + "'> </Button>"
							+ "<div class='labell' id = 'label_" + childIdL + "_L' ><div class='sublabell'><p id='desc" + childIdL + "L'><b><i>" + catgarray[catg].name + "</b></i>:" + catgarray[catg].description
							...
							+ catgarray[catg].ammount + "</span> <label id = 'rate" + childIdL + "lb'> Rate:</label> <span id = 'rate" + childIdL + "L'>" + catgarray[catg].rate + "</span></div></div>";
						...
						if (grandChildList != null && grandChildList.length > 0) {
							...
							for (let gchild in grandChildList) {
								...
								grandchildtxt_1 += "<Button id = 'handle" + gChildId + "_" + "L' class = 'subhandleL' ondblclick='toggleVisibility(&#39ggrandchild" + gChildId + x + "&#39)'> </Button>" +
									"<div class='labell' id = 'label" + gChildId + "_" + "L' ><div class='labell'><p id='desc" + gChildId + "L'><b><i>" + grandChildList[gchild].name + "</b></i>: " + grandChildList[gchild].description
									...
									+ grandChildList[gchild].ammount + "</span><label id ='rate" + gChildId + "Lb' >Rate: </label><span id = 'rate" + gChildId + "L'>" + grandChildList[gchild].rate + "</span></div></div>";
									...
									if (gGrandChildList != null && gGrandChildList.length > 0) {
										...
										for (let ggchild in gGrandChildList) {
											...
											ggrandchildtxt += "<Button id = 'handle" + "_" + ggChildId + "_" + "L' class = 'subhandleL' value='"
												+ gGrandChildList[ggchild].name + "'> </Button>" + "<div class='labell' id = 'label_" + secondArray[x].value + "_" + catgarray[catg].value + "_" + grandChildList[gchild].value + "_" + gGrandChildList[ggchild].value + "_L' ><div class='labell'><p><b><i>"
												...
												+ " Rate: " + gGrandChildList[ggchild].rate + "</p></div></div>";
											ggrandchildtxt += "</div>";
			...
			for (let target in targetArray) {
				...
				textRight += "<div id='mybutton_" + targetArray[target].value + "R' class='mybuttonR' draggable='true' ondragstart='dragged(event)' style='top:" + topRightVal + "px;'>";
				...
				textRight += "<Button id = 'parentHandle" + target + "R' class = 'handleR' value='" + targetArray[target].value + "' ondblclick='toggleVisibility(&#39" + id.trim() + "&#39)'></button><div id = 'parentLabel"
					...
					+ "</p><br><label id = 'qty" + targetArray[target].value + "LbR'>Qty.: </label> <span id = 'qty" + targetArray[target].value + "R'> " + targetArray[target].quantity
					+ "</span><br> <label id = 'amt" + targetArray[target].value + "LbR'> Total ammount: Rs.</label> <span id = 'amt" + targetArray[target].value + "R'> "
					+ targetArray[target].ammount + "</span> <br><label id = 'rate" + targetArray[target].value + "LbR'> Rate:</label> <span id = 'rate" + targetArray[target].value + "R'>" + targetArray[target].rate + "</span></div>";
				...
				if (rightsubcategorry != null && rightsubcategorry.length > 0) {
					...
					for (let catg in rightsubcategorry) {
						...
						let subtxtRight_1 = "<div id='myHiddenDiv" + rightGrandChildId
							+ "R' class='myHiddenDivR' ondragstart='dragged(event)' style='top:" + grandChildTop + "px;left:" + childLeft + "px;'>";
						subtxtRight_1 += "<Button id = 'handle_" + rightGrandChildId
							+ "_R' class = 'handleR' ondblclick='toggleVisibility(&#39grandchildHiddenDiv_" + rightGrandChildId + "R&#39)' value='"
							...
							+ " <br></p><br><label id = 'qty" + rightGrandChildId
							+ "LbR'>Qty.: </label> <span id = 'qty" + rightGrandChildId + "R'> "
							+ rightsubcategorry[catg].quantity
							+ "</span><br> <label id = 'amt" + rightGrandChildId
							...
							+ " </span> <br><label id = 'rate" + rightGrandChildId
							+ "LbR'> Rate:</label> <span id = 'rate" + rightGrandChildId
							+ "R'> " + rightsubcategorry[catg].rate + "</span></div>";

							console.log("right Grand Child : ",rightGrandChild);
						//grandChild
						if (rightGrandChild != null && rightGrandChild.length > 0) {
							let grandChildR = "<div id = 'grandchildHiddenDiv_" + rightGrandChildId + "R' style='display: none; position: absolute; top: 0px; left: -80px;  border: 1px solid blue;'>";
							...
							for (let rGrandChild in rightGrandChild) {
								...
								rGrandChild_1 += "<Button id = 'handle_" + target + "." + rGrandChild + "_R' class = 'handleR' value='"
									...
									+ "</span><br> <label id = 'amt" + rightGtGrandChildId + "LbR'> Total ammount: Rs.</label> <span id = 'amt" + rightGtGrandChildId + "R'> " + rightsubcategorry[catg].ammount
									+ " </span> <br><label id = 'rate" + rightGtGrandChildId + "LbR'> Rate:</label> <span id = 'rate" + rightGtGrandChildId + "R'> " + rightsubcategorry[catg].rate + "</span></div>";
								rGrandChild_1 += "</div>";
								grandChildR += rGrandChild_1;
								grandchildR_top = grandchildR_top + 110;
